...
fprintf('===== 3: dd = sqrt(6)*(rO+rB) + w1*deltaA + w2*(deltaA^2) + w3*ENA + w4*(ENA^2) + w5*(deltaA*ENA) =====\n');
...
features_matrix = [deltaA, deltaA.^2, ENA, ENA.^2, deltaA .* ENA];
...
figure;
h = heatmap(correlation_matrix, 'XDisplayLabels', {'\delta_{A}', '\delta_{A}^2', 'EN_{A}', 'EN_{A}^2', 'delta_{A}*EN_{A}'}, 'YDisplayLabels', {'\delta_{A}', 'delta_{A}^2', 'EN_{A}', 'EN_{A}^2', 'delta_{A}*EN_{A}'});
...
fprintf('\n===== VIF =====\n');
X_for_vif = X_dd;
var_names = {'deltaA', 'delta^A', 'ENA', 'ENA^2', 'deltaA*ENA'};
