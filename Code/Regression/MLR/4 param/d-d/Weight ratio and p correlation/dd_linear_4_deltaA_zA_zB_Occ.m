...
fprintf('ZA: %.6f%%\n', normalized_contribution(2));     % ZA
fprintf('ZB: %.6f%%\n', normalized_contribution(3));     % ZB
fprintf('Occ: %.6f%%\n', normalized_contribution(4));    % Occ
...
% ========== 输出结果 ==========
fprintf('===== 公式 3: dd = sqrt(6)*(rO+rB) + w1*deltaA + w2*ZA + w3*ZB + w4*Occ =====\n');
...
features_matrix = [deltaA, ZA, ZB, Occ];
...
% 绘制相关性热图
figure;
h = heatmap(correlation_matrix, 'XDisplayLabels', {'\delta_{A}', 'z_{A}', 'z_{B}', 'Occ'}, 'YDisplayLabels', {'\delta_{A}', 'z_{A}', 'z_{B}', 'Occ'});
...
xticklabels({'\delta_A', 'Z_A', 'Z_B', 'Occ'});
ylabel(' (%)', 'FontName', 'Arial');
title('dd', 'FontName', 'Arial');
grid on;


T = table(deltaA, ZA, ZB, Occ, dd_values, ...
    'VariableNames', {'deltaA', 'ZA', 'ZB', 'Occ', 'dd'});
