ilePath = 'E:\Hollandite\Hollandite_data.xlsx';
...
fprintf('===== 2: c = sqrt(2)*(rO+rB) + w1*deltaA + w2*deltaB + w3*ZA + w4*ZB + w5*ENA + w6*ENB + w7*Occ =====\n');
...
features_matrix = [deltaA, deltaB, ZA, ZB, ENA, ENB, Occ];
...
figure;
h = heatmap(correlation_matrix, 'XDisplayLabels', {'\delta_{A}', '\delta_{B}', 'z_{A}', 'z_{B}', 'EN_{A}', 'EN_{B}', 'Occ'}, 'YDisplayLabels', {'\delta_{A}', '\delta_{B}', 'z_{A}', 'z_{B}', 'EN_{A}', 'EN_{B}', 'Occ'});
...
xticklabels({'\delta_A', '\delta_B', 'Z_A', 'Z_B', 'EN_A', 'EN_B', 'Occ'});
ylabel(' (%)', 'FontName', 'Arial');
title('', 'FontName', 'Arial');
grid on;


T = table(deltaA, deltaB, ZA, ZB, ENA, ENB, Occ, c_values, ...
    'VariableNames', {'deltaA', 'deltaB', 'ZA', 'ZB', 'ENA', 'ENB', 'Occ', 'c'});
...
fprintf('\n===== VIF =====\n');
X_for_vif = X_c;
var_names = {'deltaA', 'deltaB', 'ZA', 'ZB', 'ENA', 'ENB', 'Occ'};
