...
fprintf('===== 3: dd = sqrt(6)*(rO+rB) + w1*deltaA + w2*ZA + w3*ZB =====\n');
...
figure;
h = heatmap(correlation_matrix, 'XDisplayLabels', {'\delta_{A}', 'z_{A}', 'z_{B}'}, 'YDisplayLabels', {'\delta_{A}', 'z_{A}', 'z_{B}'});
...
xticklabels({'\delta_A', 'Z_A', 'Z_B'});
ylabel(' (%)', 'FontName', 'Arial');
title('dd', 'FontName', 'Arial');
grid on;


T = table(deltaA, ZA, ZB, dd_values, ...
    'VariableNames', {'deltaA', 'ZA', 'ZB', 'dd'});
