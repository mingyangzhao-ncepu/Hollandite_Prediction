...
fprintf('===== 3: dd = sqrt(6)*(rO+rB) + w1*deltaA + w2*ZA + w3*ZB + w4*ENA + w5*ENB =====\n');
...
features_matrix = [deltaA, ZA, ZB, ENA, ENB];
...
figure;
h = heatmap(correlation_matrix, 'XDisplayLabels', {'\delta_{A}', 'z_{A}', 'z_{B}', 'EN_{A}', 'EN_{B}'}, 'YDisplayLabels', {'\delta_{A}', 'z_{A}', 'z_{B}', 'EN_{A}', 'EN_{B}'});
...
xticklabels({'\delta_A', 'Z_A', 'Z_B', 'EN_A', 'EN_B'});
ylabel('', 'FontName', 'Arial');
title('dd', 'FontName', 'Arial');
grid on;


T = table(deltaA, ZA, ZB, ENA, ENB, dd_values, ...
    'VariableNames', {'deltaA', 'ZA', 'ZB', 'ENA', 'ENB', 'dd'});
...
fprintf('\n===== VIF  =====\n');
X_for_vif = X_dd;
var_names = {'deltaA', 'ZA', 'ZB', 'ENA', 'ENB'};
