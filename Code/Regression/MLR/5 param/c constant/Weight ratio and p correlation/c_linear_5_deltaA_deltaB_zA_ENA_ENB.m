...
fprintf('===== c = sqrt(2)*(rO+rB) + w1*deltaA + w2*deltaB + w3*ZA + w4*ENA + w5*ENB =====\n');
...
features_matrix = [deltaA, deltaB, ZA, ENA, ENB];
...
figure;
h = heatmap(correlation_matrix, 'XDisplayLabels', {'\delta_{A}', '\delta_{B}', 'z_{A}', 'EN_{A}', 'EN_{B}'}, 'YDisplayLabels', {'\delta_{A}', '\delta_{B}', 'z_{A}', 'EN_{A}', 'EN_{B}'});
...
xticklabels({'\delta_A', '\delta_B', 'Z_A', 'EN_A', 'EN_B'});
ylabel('', 'FontName', 'Arial');
title('', 'FontName', 'Arial');
grid on;


T = table(deltaA, deltaB, ZA, ENA, ENB, c_values, ...
    'VariableNames', {'\delta_A', '\delta_B', 'Z_A', 'EN_A', 'EN_B', 'c'});
...
X_for_vif = X_c;
var_names = {'\delta_A', '\delta_B', 'Z_A', 'EN_A', 'EN_B'};
