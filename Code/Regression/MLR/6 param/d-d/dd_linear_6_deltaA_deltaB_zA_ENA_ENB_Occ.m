...
fprintf('===== 3: dd = sqrt(6)*(rO+rB) + w1*deltaA + w2*deltaB + w3*ZA + w4*ENA + w5*ENB + w6*Occ =====\n');
...
features_matrix = [deltaA, deltaB, ZA, ENA, ENB, Occ];
...
figure;
h = heatmap(correlation_matrix, 'XDisplayLabels', {'\delta_{A}', '\delta_{B}', 'z_{A}', 'EN_{A}', 'EN_{B}', 'Occ'}, 'YDisplayLabels', {'\delta_{A}', '\delta_{B}', 'z_{A}', 'EN_{A}', 'EN_{B}', 'Occ'});
...
T = table(deltaA, deltaB, ZA, ENA, ENB, Occ, dd_values, ...
    'VariableNames', {'deltaA', 'deltaB', 'ZA', 'ENA', 'ENB', 'Occ', 'dd'});
...
fprintf('\n===== VIF =====\n');
X_for_vif = X_dd;
var_names = {'deltaA', 'deltaB', 'ZA', 'ENA', 'ENB', 'Occ'};
